...
def _make_dirs(out_dir: str, phase: List[str],  classes: List[str], allow_overwrite: bool):
    ...
    for p in phase:
        for c in classes:
            phase_dir = osp.join(out_dir, p, c)
...
def load_original_annotations(data_root: str,
                              ...
                              c: str = '',
                              nproc: int = 8):
    img_dir = osp.join(data_root, phase, c, 'images')
    assert osp.isdir(img_dir), f'The {img_dir} is not an existing dir!'

    if phase == 'test':
        ann_dir = None
    else:
        ann_dir = osp.join(data_root, phase, c, ann_subdir)
        ...
        with open(txtfile) as f:
            for line in f:
                ...
                if len(items) < 9:
                    ...
                    diffs.append(int(items[5]) if len(items) == 6 else 0)
                    
                if len(items) >= 9:
                    bboxes.append([float(i) for i in items[:4]])
...
def main():
    ...
    # make dirs
    _make_dirs(args.out_dir, args.phase, args.classes, args.overwrite)
    ...
    for p in args.phase:
        for c in args.classes:
            save_imgs_dir = osp.join(args.out_dir, p, c, 'images')
            save_anns_dir = osp.join(args.out_dir, p, c, 'annfiles')
