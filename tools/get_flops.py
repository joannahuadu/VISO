...
def inference(args, logger):
    ...
    if cfg.get('work_dir', None) is None:
        ...
        cfg.work_dir = osp.join('./work_dirs',
                                osp.splitext(osp.basename(args.config))[0], 'infos')
    ...
    custom_ops = {}
    custom_ops[spconv.SubMConv2d] = count_submconv2d
    custom_ops[spconv.SparseSequential] = zero_ops
    custom_ops[torch.nn.modules.activation.SiLU] = count_prelu
    custom_ops[mmcv.cnn.bricks.wrappers.Linear] = count_linear
    for idx, data_batch in enumerate(data_loader):
        ...
        macs, params = profile(model, inputs=(data['inputs'], ), custom_ops=custom_ops, verbose=False)
        avg_flops.append(macs)
        avg_params.append(params)
    ...
    # mean_flops = _format_size(np.mean(avg_flops))
    # params = _format_size(params)
    mean_flops, params = clever_format([np.average(avg_flops), params], "%.3f")
