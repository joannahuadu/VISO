...
import cv2
from .yolo_world_rotated_head import YOLOWorldRotatedHead, YOLOWorldRotatedHeadModule, YOLOv8RotatedHead, RepYOLOWorldRotatedHeadModule
try:
...
@MODELS.register_module()
class YOLOv8RotatedHeadSPInfer(YOLOv8RotatedHead):
    """YOLOv8 Rotated Head with sparse conv
    """
    def __init__(self, 
                *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
    ...
    def forward(self, img_feats: Tuple[Tensor]) -> Tuple[List]:
        """Forward features from the upstream network."""
        return self.head_module(img_feats[:self.num_levels], img_feats[self.num_levels:])
...
@MODELS.register_module()
class RepYOLOWorldRotatedHeadModuleSPInfer(RepYOLOWorldRotatedHeadModule):
    ...
    def __init__(self,
        ...
        self.is_sparse_levels = is_sparse_levels
        self.sp_type = sp_type
        self.sparse_module_name = ['cls_preds', 'reg_convs', 'reg_preds', 'ang_preds', 'cls_contrasts']
        self.sparse_module_list = [getattr(self, name) for name in self.sparse_module_name]

    def forward_single(self, img_feat: Tensor, img_attn: Tensor, is_sparse: bool, cls_pred: nn.ModuleList,
    ...
    def forward(self, img_feats: Tuple[Tensor], img_attns: Tuple[Tensor]) -> Tuple[List]:
        assert len(img_feats) == self.num_levels
        return multi_apply(self.forward_single, img_feats, img_attns, self.is_sparse_levels, self.cls_preds,
