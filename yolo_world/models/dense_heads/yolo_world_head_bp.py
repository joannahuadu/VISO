...
@MODELS.register_module()
class YOLOWorldRotateHeadModule(YOLOv8HeadModule):
    ...
    def _init_layers(self) -> None:
        ...
        reg_out_channels = max(
            (16, self.in_channels[0] // 5, self.reg_max * 5))
        cls_out_channels = max(self.in_channels[0], self.num_classes)
    ...
    def forward_single(self, img_feat: Tensor, txt_feat: Tensor,
        ...
        if self.reg_max > 1:
            bbox_dist_preds = bbox_dist_preds.reshape(
                [-1, 5, self.reg_max, h * w]).permute(0, 3, 1, 2)
...
@MODELS.register_module()
class YOLOWorldRotateHead(YOLOWorldHead):
    """YOLO-World Head
    """
    ...
    def loss_by_feat(
        ...
        flatten_pred_bboxes = [
            bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, self.bbox_coder.encode_size)
            for bbox_pred in bbox_preds
        ]
        # (bs, n, 4 * reg_max)
        flatten_pred_dists = [
        ...
        if num_pos > 0:
            ...
            # will not report an error
            # iou loss
            prior_bbox_mask = fg_mask_pre_prior.unsqueeze(-1).repeat([1, 1, self.bbox_coder.encode_size])
            pred_bboxes_pos = torch.masked_select(
                flatten_pred_bboxes, prior_bbox_mask).reshape([-1, self.bbox_coder.encode_size])
            assigned_bboxes_pos = torch.masked_select(
                assigned_bboxes, prior_bbox_mask).reshape([-1, self.bbox_coder.encode_size])
            bbox_weight = torch.masked_select(assigned_scores.sum(-1),
                                              fg_mask_pre_prior).unsqueeze(-1)
            loss_bbox = self.loss_bbox(
                pred_bboxes_pos, assigned_bboxes_pos,
            ...
            loss_dfl = self.loss_dfl(pred_dist_pos.reshape(
                -1, self.head_module.reg_max),
                                     assigned_ltrb_pos.reshape(-1),
                                     weight=bbox_weight.expand(-1,
                                                               self.bbox_coder.encode_size).reshape(-1),
                                     avg_factor=assigned_scores_sum)
        else:
            loss_bbox = flatten_pred_bboxes.sum() * 0
            loss_dfl = flatten_pred_bboxes.sum() * 0
