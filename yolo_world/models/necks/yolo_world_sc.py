...
class YOLOWorldSP(YOLOv8PAFPN):
    ...
    def forward(self, img_feats: List[Tensor], txt_feats: Tensor = None) -> tuple:
        ...
        for idx in range(len(self.in_channels)):
            reduce_outs_mask.append(self.reduce_layers[idx](img_feats[idx]))
            reduce_outs.append(reduce_outs_mask[idx]*img_feats[idx])
        # top-down path
        ...
        # add reduce_outs_mask
        for idx in range(len(self.in_channels)):
            results.append(reduce_outs_mask[idx])
            
        return tuple(results)
    
    def forward_sp(self, img_feats: List[Tensor], txt_feats: Tensor = None) -> tuple: 
        pass
