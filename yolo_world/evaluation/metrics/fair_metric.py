...
class FAIRMetric(BaseMetric):
    ...
    """
    ORI_CLASSES = ('Tugboat', 'Passenger Ship', 'Small Car', 'Dump Truck', 'Van', 'Dry Cargo Ship', 'Motorboat',
                'Engineering Ship', 'other-airplane', 'A220', 'Boeing737', 'A330', 'Boeing787', 'Boeing747',
                'A321', 'Intersection', 'Tennis Court', 'Basketball Court', 'Football Field', 'Truck Tractor',
                'Cargo Truck', 'Baseball Field', 'other-vehicle', 'Roundabout', 'A350', 'other-ship', 'Liquid Cargo Ship',
                'Bus', 'Trailer', 'Boeing777', 'ARJ21', 'Fishing Boat', 'Warship', 'C919', 'Bridge', 'Excavator', 'Tractor')
    ...
    def merge_results(self, results: Sequence[dict],
        ...
        for img_id, dets_per_cls in zip(id_list, dets_list):
            ...
            ET.SubElement(source, "filename").text = f"{img_id}.tif"
            ...
            ET.SubElement(research, "provider").text = "THU"
            ET.SubElement(research, "author").text = "BECOOL"
            ...
            for cls_idx, dets in enumerate(dets_per_cls):
                if dets.size == 0:
                    continue  # Skip if no detections for this class
                ...
                for qbox, score in zip(qboxes, scores):
                    ...
                    ET.SubElement(possibleresult, "name").text = self.ORI_CLASSES[cls_idx]  # Assuming `self.CLASSES` contains class names
                    ET.SubElement(possibleresult, "probability").text = f"{score.item():.4f}" # Detection score
                    ...
                    for point in points_list:
                        ET.SubElement(points, "point").text = point
            
            xml_tree = ET.ElementTree(annotation)
            xml_filename = osp.join(outfile_prefix, f"{img_id}.xml")
            xml_tree.write(xml_filename)
        target_name = osp.split(outfile_prefix)[-1]
        with zipfile.ZipFile(osp.join(outfile_prefix, target_name + '.zip'), 'w', zipfile.ZIP_DEFLATED) as zipf:
            for img_id in id_list:
                xml_filename = osp.join(outfile_prefix, f"{img_id}.xml")
                zipf.write(xml_filename, osp.basename(xml_filename))
        return osp.join(outfile_prefix, target_name + '.zip')
