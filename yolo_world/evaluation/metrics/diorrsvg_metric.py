...
class DIORRSVGMetric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            gt_label = data_sample['gt_instances']['labels']
            phrases = data_sample['texts']
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for pred, gt_boxes, gt_labels, phrases in zip(pred_list, gt_list,
            ...
            for i, phrase in enumerate(phrases):
                ...
                ious = box_iou_rotated(
                    torch.from_numpy(cur_boxes).float(),
                    tar_boxes).numpy()
                for k in self.topk:
                    if k == -1:
                        maxi = ious.max()
        ...
        miou = sum(total_ious) / len(total_ious)
